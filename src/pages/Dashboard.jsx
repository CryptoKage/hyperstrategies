...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    const [
      ...
      pool.query("SELECT vault_id, name, description, status, image_url FROM vaults WHERE status IN ('active', 'coming_soon') ORDER BY vault_id ASC"),
    ...
    const performanceByVault = allPerformanceDataResult.rows.reduce((acc, row) => {
        if (!acc[row.vault_id]) acc[row.vault_id] = [];
    ...
    for (const vaultId in performanceByVault) {
        ...
        if (history.length > 0) {
            ...
            // --- THE DEFINITIVE FIX: Use high-precision minutes ---
            const totalMinutesActive = moment(lastRecord.record_date).diff(moment(firstRecord.record_date), 'minutes');
            const totalDaysActive = totalMinutesActive / 1440.0;

            if (totalDaysActive < 1) {
                // If less than a day old, all averages are the total performance.
                const totalReturn = parseFloat(totalPerformancePercent.toFixed(2));
                performanceMap[vaultId] = { daily: totalReturn, weekly: totalReturn, monthly: totalReturn, total: totalReturn };
            } else {
                const avgDailyReturn = totalPerformancePercent / totalDaysActive;
                performanceMap[vaultId] = {
                    daily: parseFloat(avgDailyReturn.toFixed(2)),
                    weekly: parseFloat((avgDailyReturn * 7).toFixed(2)),
                    monthly: parseFloat((avgDailyReturn * 30.4375).toFixed(2)),
    ...
    const availableVaults = vaultsResult.rows.map(vault => ({
        ...vault,
        performance: performanceMap[vault.vault_id] || { daily: 0, weekly: 0, monthly: 0, total: 0 }
    }));
