...
const Dashboard = () => {
  ...
  // NEW: track which "Manage position" menu is open
  ...
  const handleToggleAutoCompound = async (vaultId) => {
    ...
    try {
      await api.put(`/user/vault-settings/${vaultId}/compound`, {
        autoCompound: newState,
      });
    } catch (err) {
      console.error('Failed to update auto-compound setting:', err);
      setAutoCompoundState((prevState) => ({
        ...prevState,
        [vaultId]: currentState,
      }));
    } finally {
  ...
  const handleOpenTransferModal = (position) => {
    const vaultInfo = dashboardData.vaults.find(
      (v) => v.vault_id === position.vault_id
    );
  ...
  const renderContent = () => {
    ...
    // NEW: hasAvailableBalance for onboarding logic
    const hasAvailableBalance =
      (dashboardData.availableBalance || 0) > 0.01;

    return (
      <>
        {/* Top Stats Grid (Unchanged) */}
        <div className="stats-grid">
          <div className="stat-card">
            ...
            <div className="stat-main">
              ...
              <button
                ...
                className="btn-icon"
                title={t(
                  'dashboard.toggle_balance_tooltip',
            ...
            <span
              className={`stat-sub-value ${
                (dashboardData.totalUnrealizedPnl || 0) >= 0
                  ? 'stat-pnl-positive'
                  : 'stat-pnl-negative'
              }`}
            >
              {t('dashboard.unrealized_pnl')}:{' '}
              {isBalanceHidden
                ? '******'
                : `${(dashboardData.totalUnrealizedPnl || 0) >= 0 ? '+' : ''}${(
          ...
          <div className="stat-card">
            ...
            <div className="stat-main">
              ...
              <button onClick={() => navigate('/wallet')} className="btn-link">
                {t('dashboard.manage')}
              </button>
            </div>
          </div>
          ...
          <div className="stat-card">
            ...
            <span className="stat-value">
              {isBalanceHidden
                ? '******'
                : `$${(dashboardData.totalBonusPoints || 0).toLocaleString(
                    'en-US',
                    {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
        ...
        {/* NEW CONDITIONAL LOGIC FOR ONBOARDING */}
        {investedPositions.length === 0 && !hasAvailableBalance ? (
          <WelcomeCard />
        ) : (
          investedPositions.length > 0 && (
            <>
              <h2 style={{ marginTop: '48px' }}>
                {t('dashboard.your_positions')}
              </h2>
              <div className="vaults-grid">
                {investedPositions.map((position) => {
                  ...
                  const pendingTransferOut =
                    dashboardData.pendingTransfers?.find(
                  ...
                  const cardStyle =
                    vaultInfo.image_url &&
                    vaultImageMap[vaultInfo.image_url]
                      ? {
                          backgroundImage: `url(${vaultImageMap[vaultInfo.image_url]})`,
                        }
                      : {};
                  const vaultLockStatus =
                    vaultLockStatuses[position.vault_id] || { isLocked: true };

                  return (
                    <div
                      key={position.vault_id}
                      className="vault-card-container"
                    >
                      <div
                        className="vault-card invested with-bg"
                        style={cardStyle}
                      >
                        ...
                        <div className="card-content">
                          ...
                          <div className="vault-stat">
                            ...
                            <span>
                              {isBalanceHidden
                                ? '******'
                                : `$${tradableCapital.toLocaleString('en-US', {
                                    minimumFractionDigits: 2,
                                    maximumFractionDigits: 2,
                                  })}`}
                            </span>
                          ...
                          <div className="auto-compound-toggle">
                            <div className="label-group">
                              <label htmlFor={`compound-toggle-${position.vault_id}`}>
                                {isUpdating
                                  ? t('dashboard.saving')
                                  : t('dashboard.auto_compound')}
                              </label>
                              <Link
                                to="/faq#auto-compound"
                                className="info-icon-link"
                                title={t('dashboard.auto_compound_tooltip')}
                              >
                                <InfoIcon />
                              </Link>
                            </div>
                            <label className="switch">
                              <input
                                ...
                                checked={
                                  autoCompoundState[position.vault_id] ?? true
                                }
                                onChange={() =>
                                  handleToggleAutoCompound(position.vault_id)
                                }
                                disabled={isUpdating}
                              />
                          ...
                          {/* CORRECTED BUTTONS SECTION WITH DROPDOWN */}
                            ...
                            <div className="manage-position-menu">
                              ...
                              {openMenuId === position.vault_id && (
                                <div className="manage-dropdown-content">
                                  <button
                                    onClick={() => {
                                      handleOpenAllocateModal(vaultInfo);
                                      setOpenMenuId(null);
                                    }}
                                  >
        ...
        >
            ...
            {vaultLockStatus.isLocked && <span className="lock-icon">🔒</span>}
                            ...
                            <button
                              className="btn-primary"
                              onClick={() =>
                                navigate(`/vaults/${vaultInfo.vault_id}`)
                              }
                            >
                      ...
                      {vaultInfo.performance && (
                        <div className="performance-snapshot-card">
                          <SnapshotItem
                            labelKey="vault.stats.dailyReturn"
                            value={vaultInfo.performance.daily}
                            className={
                              vaultInfo.performance.daily >= 0
                                ? 'text-positive'
                                : 'text-negative'
                            }
                          />
                          <SnapshotItem
                            labelKey="vault.stats.weeklyReturn"
                            value={vaultInfo.performance.weekly}
                            className={
                              vaultInfo.performance.weekly >= 0
                                ? 'text-positive'
                                : 'text-negative'
                            }
                          />
                          <SnapshotItem
                            labelKey="vault.stats.monthlyReturn"
                            value={vaultInfo.performance.monthly}
                            className={
                              vaultInfo.performance.monthly >= 0
                                ? 'text-positive'
                                : 'text-negative'
                            }
                          />
                          <SnapshotItem
                            labelKey="vault.stats.totalReturn"
                            value={vaultInfo.performance.total}
                            className={
                              vaultInfo.performance.total >= 0
                                ? 'text-positive'
                                : 'text-negative'
                            }
                          />
              ...
              >
                ...
                {isActive && vault.performance && (
                  <div className="performance-snapshot-card">
                    ...
                    <SnapshotItem
                      labelKey="vault.stats.weeklyReturn"
                      value={vault.performance.weekly}
                      className={
                    ...
                    <SnapshotItem
                      labelKey="vault.stats.totalReturn"
                      value={vault.performance.total}
                      className={
                        vault.performance.total >= 0
                          ? 'text-positive'
                          : 'text-negative'
                      }
                    />
